"""CLI entry point: python -m ascent_load input.bdf"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog='ascent-load',
        description='ASCENT-Load - Finite Element Analysis Solver')
    ...
    parser.add_argument('--save-results', type=str, default=None, metavar='FILE',
                        help='Save results to .aload file for later visualization')
    parser.add_argument('--save', action='store_true',
                        help='Auto-save results to <input>.aload')
    parser.add_argument('--spline-slope', default='surface',
                        choices=['rotation', 'surface'],
                        help='Normalwash slope construction for surface '
                             'splines: surface=analytic derivative of the '
                             'IPS displacement surface (SPLINE1 semantics, '
                             'default), rotation=nodal theta_y (SPLINE2-'
                             'like, for reproducing pre-change results)')
    parser.add_argument('--pload4', action='store_true',
                        help='Also export trim aero box forces as PLOAD4 '
                             'skin-element pressures (<input>_pload4.bdf, '
                             'per-subcase SIDs) — auxiliary product for '
                             'skin-panel local design')
    parser.add_argument('--no-w2gj', action='store_true',
                        help='Ignore the deck W2GJ DMI (camber/twist '
                             'downwash); flat-plate panels, for '
                             'reproducing results archived before W2GJ '
                             'support')
    ...
    if bdf_model.sol == 101:
        ...
        results = solve_trim(bdf_model,
                             ...
                             spline_slope_method=args.spline_slope,
                             apply_w2gj=not args.no_w2gj)
        ...
        logger.info("  Use: from ascent_load.solvers.sol146 import "
    ...
    # Save results to .aload file
    save_path = args.save_results or (str(bdf_path.with_suffix('.aload')) if args.save else None)
    if save_path:
    ...
    # PLOAD4 skin pressures (auxiliary; SOL 144 only)
    if getattr(args, 'pload4', False) and bdf_model.sol == 144 \
        ...
        pload_path = str(bdf_path.with_suffix('')) + '_pload4.bdf'
        export_pload4(bdf_model, boxes, results, pload_path)

    # Visualization screenshot
    if args.screenshot and bdf_model.sol == 144 and results.subcases:
        try:
            from .visualization.viewer import AscentLoadViewer
            viewer = AscentLoadViewer(bdf_model, results, off_screen=True)

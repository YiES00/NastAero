...
class LandingGearConfig:
    ...
    tire_deflection: float = 0.0     # 타이어 압착 처짐 (m); LGFACTOR의 STIRE
    tire_efficiency: float = 0.3     # 타이어 에너지 흡수 효율 (LGFACTOR ETIRE)

    def landing_load_factors(self, weight_N: float,
                             wing_area_m2: float = 0.0,
                             lift_factor: float = 2 / 3,
                             ) -> tuple:
        """§23.473 착륙 하중배수 — FAR 23 LOADS Manual Ch.20 (LGFACTOR.BAS).

        침하속도: V = 4.4·(W/S)^(1/4) [ft/s], 7~10 ft/s 클램프
        (§23.473(d)); ``sink_rate_fps`` > 0이면 그 값을 같은 범위로
        클램프해 사용한다.

        에너지법 (LGFACTOR):
            N = ( V²/(2g) + (1−L)·(d_s + d_t) ) / ( η_s·d_s + η_t·d_t )
        여기서 L=양력비(≤2/3, §23.473(e)), d_s=스트로크, d_t=타이어
        처짐, η=흡수 효율. 하한 §23.473(g): N ≥ 2.67, N−L ≥ 2.0.

        Returns
        -------
        (N, N_gear, V_fps) : tuple of float
            기체 관성 하중배수, 지면반력 하중배수(N − L), 침하속도.
        """
        # 침하속도 (ft/s)
        if self.sink_rate_fps > 0:
            v_fps = self.sink_rate_fps
        elif wing_area_m2 > 0:
            w_lb = weight_N / 4.44822
            s_ft2 = wing_area_m2 / 0.09290304
            v_fps = 4.4 * (w_lb / s_ft2) ** 0.25
        else:
            v_fps = 10.0
        v_fps = min(max(v_fps, 7.0), 10.0)

        lift = min(lift_factor, 2 / 3)
        if self.stroke <= 0:
            n = 3.0                      # 보수적 기본값
        else:
            v = v_fps * FPS_TO_MPS
            d_s, d_t = self.stroke, max(self.tire_deflection, 0.0)
            absorbed = self.strut_efficiency * d_s + self.tire_efficiency * d_t
            n = (v ** 2 / (2 * G_MPS2) + (1 - lift) * (d_s + d_t)) / absorbed

        n = max(n, 2.67, 2.0 + lift)     # §23.473(g)
        return n, n - lift, v_fps

    def compute_nz_landing(self, weight_N: float, lift_factor: float = 2 / 3) -> float:
        """(하위 호환) 기체 관성 하중배수 N만 반환한다."""
        return self.landing_load_factors(weight_N, lift_factor=lift_factor)[0]
    ...
    def from_dict(cls, d: dict) -> AircraftConfig:
        ...
        if 'landing_gear' in d:
            ...
            cfg.landing_gear = LandingGearConfig(
                ...
                tire_deflection=lg.get('tire_deflection', 0.0),
                tire_efficiency=lg.get('tire_efficiency', 0.3),
            )

...
- Saves/resumes from .aload checkpoint files
...
from .landing_loads import (
    ...
    compute_landing_inertial_forces, compute_landing_lift_forces,
    combine_forces,
)
...
class BatchRunner:
    ...
    def _solve_landing_case(self, cond: LandingCondition) -> CaseResult:
        ...
        if self.bdf_model is not None:
            # 관성 질량은 CONM2 + 구조(요소) 질량을 모두 럼핑한
            # compute_node_masses 기준 — 관성/트림과 단일 소스
            from ..trim_loads import (
                apply_inertia_relief, compute_node_masses,
            )
            from ...solvers.sol144 import _compute_cg

            node_masses = compute_node_masses(self.bdf_model)

            # Detect gravity from model unit system
            g = self._detect_gravity()
            inertial = compute_landing_inertial_forces(
                cond, node_masses, g=g)
            # §23.473(e): 착륙 충격 중 양력 lift_factor·W (질량 비례
            # 분포) — 지면반력 (nz−L)·W와 함께 ΣFz가 정확히 닫힌다
            lift = compute_landing_lift_forces(cond, node_masses, g=g)
            nodal_forces = combine_forces(gear_forces, inertial, lift)
            # 착륙은 각가속도가 발생하는 과도 상태 — 기어 반력과
            # 관성의 잔여 모멘트를 강체 가속도 관성력으로 흡수해
            # 자기평형 하중으로 만든다 (inertial 인자는 더미 {}).
            relief = apply_inertia_relief(
                self.bdf_model, {}, nodal_forces,
                cg=_compute_cg(self.bdf_model), g=g)
            flight_state = dict(relief)
    ...
    def _save_checkpoint(self) -> None:
        ...
        # Save metadata (not nodal forces — those go to .aload)
        data = {

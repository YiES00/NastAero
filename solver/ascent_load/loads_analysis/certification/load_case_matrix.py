...
Dynamic (6-DOF simulation → critical time extraction):
- Elevator pull-up/push-over (§23.331(c))
- Checked maneuver doublet (§23.331(c))
- Abrupt aileron roll (§23.349)
- Rudder yaw maneuver (§23.351)
- Discrete vertical/lateral gust (§23.341)
...
# sim_runner and sim_to_loads are imported lazily inside
# _add_dynamic_maneuver_cases() to avoid circular imports
# (sim_to_loads imports CertLoadCase from this module).
...
class LoadCaseMatrix:
    ...
    def generate_all(self, bdf_model=None, include_dynamic: bool = True,
                      dynamic_n_workers: int = 0,
                      dynamic_max_cases: int = 15) -> None:
        """Generate all certification load cases.

        Populates self.flight_cases, self.dynamic_cases, and
        self.landing_cases.

        Parameters
        ----------
        bdf_model : BDFModel, optional
            Parsed BDF model.  Required when ``include_dynamic=True``
            so that VLM stability derivatives can be computed for the
            6-DOF maneuver simulations.
        include_dynamic : bool
            If True (default), run 6-DOF dynamic maneuver simulations
            (elevator pull-up, checked, abrupt roll, yaw, discrete
            gust) to extract critical time points and convert them to
            additional SOL 144 load cases.  Set False to skip (faster,
            quasi-static cases only).
        dynamic_n_workers : int
            Number of parallel workers for dynamic simulations.
            0 = auto (os.cpu_count()).
        dynamic_max_cases : int
        ...
        for wc in self.config.weight_cg_conditions:
            for alt_m in self.config.altitudes_m:
                ...
                # Flight envelope cases (quasi-static V-n corners)
        ...
        # Dynamic maneuver cases: 6-DOF sim → critical time → SOL 144
        if include_dynamic and bdf_model is not None:
            self._add_dynamic_maneuver_cases(
                bdf_model, dynamic_n_workers, dynamic_max_cases)
    ...
    # ---------------------------------------------------------------
    # Dynamic maneuver simulation — §23.331(c)/§23.341/§23.349/§23.351
    # ---------------------------------------------------------------

    def _add_dynamic_maneuver_cases(self, bdf_model,
                                      n_workers: int,
                                      max_cases: int) -> None:
        """Run 6-DOF dynamic maneuver simulations and add critical cases.

        Workflow:
        1. Compute VLM stability derivatives from BDF model
        2. Generate all FAR 23 maneuver profiles (elevator, checked,
           roll, yaw, vertical gust, lateral gust)
        3. Run 6-DOF RK4 simulations in parallel
        4. Extract critical time points (max/min nz, alpha, beta, p, q, r)
        5. Deduplicate by farthest-point sampling in 10-D flight state space
        6. Convert to CertLoadCase with zero-free-variable SOL 144 trim

        Critical time points capture transient peak responses that
        quasi-static V-n corner methods cannot reproduce (e.g., pitch
        overshoot during abrupt checked maneuver, peak roll rate from
        aileron input, gust transient peak).
        """
        import logging
        logger = logging.getLogger(__name__)
        logger.info("Running 6-DOF dynamic maneuver simulations...")

        # Lazy import to break circular dependency
        # (sim_to_loads imports CertLoadCase from this module)
        from .sim_runner import SimRunner
        from .sim_to_loads import critical_points_to_load_cases

        try:
            runner = SimRunner(
                config=self.config, bdf_model=bdf_model,
                n_workers=n_workers)
            results, info = runner.run_all()

            n_crits = sum(len(r.critical_points) for r in results)
            logger.info("  %d simulations, %d critical time points extracted",
                        info.get('n_sims', 0), n_crits)

            if n_crits == 0:
                logger.warning("  No critical points found — skipping dynamic cases")
                return

            # Convert to CertLoadCase (includes deduplication)
            dyn_cases = critical_points_to_load_cases(
                results, self.config,
                case_id_start=10000,
                max_dynamic_cases=max_cases,
            )

            self.dynamic_cases = dyn_cases
            logger.info("  %d dynamic load cases added (case_id 10000+)", len(dyn_cases))

        except Exception as e:
            logger.warning("  Dynamic maneuver simulation failed: %s — "
                           "continuing with quasi-static cases only", e)

...
compared against ASCENT-Load's FEM-based SOL 144 aeroelastic trim results.
...
# ═══════════════════════════════════════════
# Comparison with ASCENT-Load
# ═══════════════════════════════════════════

def compare_with_ascent_load(
    far23: FAR23Results,
    ascent_load_vn,
    ascent_load_config
) -> Dict:
    """Compare FAR 23 analytical results with ASCENT-Load results.

    Args:
        far23: FAR23Results from compute_far23_loads
        ascent_load_vn: ASCENT-Load VnDiagram object
        ascent_load_config: ASCENT-Load AircraftConfig object
    ...
    comparison["design_speeds"] = {
        "VA": {"far23": far23.VA, "ascent_load": ascent_load_config.speeds.VA},
        "VC": {"far23": 0, "ascent_load": ascent_load_config.speeds.VC},
        "VD": {"far23": 0, "ascent_load": ascent_load_config.speeds.VD},
    }

    # Compare load factors
    comparison["load_factors"] = {
        "nz_max": {"far23": far23.nz_max, "ascent_load": ascent_load_vn.nz_max},
        "nz_min": {"far23": far23.nz_min, "ascent_load": ascent_load_vn.nz_min},
    }
    ...
    for pt in far23.vn_points:
        ...
        # Find matching ASCENT-Load point
        for na_pt in ascent_load_vn.corner_points:
            if _labels_match(pt.label, na_pt.label):
                entry["ascent_load_V"] = na_pt.V_eas
                entry["ascent_load_nz"] = na_pt.nz
                break
...
def _labels_match(far23_label: str, ascent_load_label: str) -> bool:
    ...
    return mapping.get(far23_label, far23_label) == ascent_load_label

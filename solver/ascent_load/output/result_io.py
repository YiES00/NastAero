"""Result file I/O for ASCENT-Load (.aload format).
...
Usage:
    # Save after solving
    save_results(results, bdf_model, "model.aload")

    # Load for visualization (no BDF parsing or solving needed)
    results, viz_model = load_results("model.aload")
    viewer = AscentLoadViewer(viz_model, results)
"""
...
class VizModel:
    ...
    Provides the minimal interface required by AscentLoadViewer and
...
def save_results(
    ...
    """Save analysis results and model geometry to .aload file.
    ...
    filepath : str
        Output file path (recommended extension: .aload).
    bdf_file : str
        Original BDF filename (for metadata only).
    """
    filepath = str(filepath)
    # 개명 전 확장자 .naero 로 지정된 경로는 그대로 쓴다 (이중 확장 방지).
    # 읽기(load_results)는 zip 경로를 그대로 열므로 확장자와 무관하며,
    # 기존에 보관된 .naero 결과 파일은 변환 없이 계속 열린다.
    if not filepath.endswith(('.aload', '.naero')):
        filepath += '.aload'
    ...
    metadata = {
        ...
        'ascent_load_version': __version__,
...
def load_results(filepath: str) -> Tuple[ResultData, VizModel]:
    """Load results and model proxy from .aload file.
    ...
    with zipfile.ZipFile(filepath, 'r') as zf:
        ...
        if version > FORMAT_VERSION:
            raise ValueError(
                f"Unsupported .aload format version {version} "

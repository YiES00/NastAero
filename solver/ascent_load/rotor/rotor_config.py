...
class VTOLConfig:
    ...
    @classmethod
    def uam_lift_cruise(cls) -> VTOLConfig:
        """Create GACOMP Lift+Cruise default configuration.
        ...
        Wing span positions correspond to GACOMP wing stations.
        """
        ...
        # GACOMP wing: Y = 465mm (root) to 5617mm (tip)
    ...
    @classmethod
    def nasa_lift_cruise_8(cls) -> VTOLConfig:
        """Create NASA Lift+Cruise Reference Vehicle configuration (approximate).

        Based on Silva et al. (2018) NASA TM, "VTOL Urban Air Mobility
        Concept Vehicles for Technology Development", and Johnson et al.
        (2018), "Concept Vehicles for VTOL Air Taxi Operations".

        Configuration:
            8 lift rotors on 4 booms (2 per boom: front + rear)
            + 1 pusher propeller at tail

        Layout (top view, Y-axis = spanwise; X-axis = aft):
            Front-Left boom:  L1F (front), L1R (rear)
            Mid-Left boom:    L2F (front), L2R (rear)
            Mid-Right boom:   R2F (front), R2R (rear)
            Front-Right boom: R1F (front), R1R (rear)
            Tail: Pusher (P1)

        IMPORTANT — Rotor radius caveat:
            The NASA L+C concept vehicle has gone through several iterations.
            Different NASA technical memoranda and presentations cite
            slightly different rotor sizes. Reported values include:
              - "lift rotor with ~5 ft (1.524 m) diameter"
                (i.e., r ≈ 0.762 m) in the original 2018 NASA TM
              - r ≈ 0.75 m in some derivative analyses
              - r ≈ 1.0 m in later concept iterations

            This implementation uses r = 0.75 m as a reasonable approximation
            for academic-level analysis. For precise comparison with NASA L+C
            performance data, the user should verify the exact rotor radius
            against the specific NASA TM revision being referenced.

            The dissertation's primary case study uses uam_lift_cruise()
            (6 lift rotors, r=0.6 m), which is independent of NASA L+C
            specifications. This nasa_lift_cruise_8() config is provided
            as a future-work reference for those wishing to align with
            NASA's published 8-rotor concept vehicle.

        References:
            Silva, C., Johnson, W., Solis, E., Patterson, M.D., Antcliff,
              K.R. (2018). "VTOL Urban Air Mobility Concept Vehicles for
              Technology Development." NASA Technical Memorandum.
            Johnson, W., Silva, C., Solis, E. (2018). "Concept Vehicles
              for VTOL Air Taxi Operations." AHS International Forum.

        Returns
        -------
        VTOLConfig with 8 lift rotors + 1 pusher.
        """
        # Lift rotor blade (NASA L+C approximate)
        # See docstring CAVEAT above regarding rotor radius uncertainty
        lift_blade = BladeDef(
            radius=0.75,    # Approximate; verify against specific NASA TM
            root_cutout=0.15,
            n_elements=20,
            mean_chord=0.06,
            twist_root=np.radians(13.0),
            twist_tip=np.radians(3.0),
            airfoil=RotorAirfoil.naca0012(),
        )
        ...
        # 4 booms: 2 inboard (closer to fuselage) + 2 outboard
        # Y positions in mm (spanwise from centerline)
        # NASA L+C wing span ~13.7 m → boom positions
        y_inner = 2200.0   # Inner booms
        y_outer = 4200.0   # Outer booms

        # X positions in mm (front and rear of each boom)
        x_front = 3000.0   # Forward rotor on each boom
        x_rear = 6000.0    # Rear rotor on each boom

        z_boom = 700.0     # Boom Z-position (above wing)

        rotors = []
        rotor_id = 1

        # Define 4 booms × 2 rotors each = 8 lift rotors
        boom_positions = [
            (-y_outer, "L1"),  # Left outer boom
            (-y_inner, "L2"),  # Left inner boom
            (y_inner, "R2"),   # Right inner boom
            (y_outer, "R1"),   # Right outer boom
        ]

        for boom_y, boom_label in boom_positions:
            for x_pos, fr_label in [(x_front, "F"), (x_rear, "R")]:
                # Alternate rotation direction for torque balance
                rot_dir = (RotationDir.CW
                           if (rotor_id % 2 == 0)
                           else RotationDir.CCW)
                rotors.append(RotorDef(
                    rotor_id=rotor_id,
                    label=f"Lift Rotor {boom_label}{fr_label}",
                    rotor_type=RotorType.LIFT,
                    hub_position=np.array([x_pos, boom_y, z_boom]),
                    shaft_axis=np.array([0., 0., 1.]),
                    blade=lift_blade,
                    n_blades=5,    # NASA L+C uses 5 blades
                    rotation_dir=rot_dir,
                    rpm_hover=2400.0,    # NASA L+C nominal
                    rpm_cruise=0.0,      # Stopped in cruise
                    hub_node_id=990000 + rotor_id,
                    mass_kg=18.0,        # Slightly heavier than GACOMP
                    can_fail=True,
                ))
                rotor_id += 1
        ...
        rotors.append(RotorDef(
            ...
            label="Pusher P1",
            rotor_type=RotorType.CRUISE,
            hub_position=np.array([12000.0, 0.0, 1500.0]),
            shaft_axis=np.array([-1., 0., 0.]),
            blade=pusher_blade,
            n_blades=4,    # NASA L+C 4-blade pusher
            rotation_dir=RotationDir.CW,
            rpm_hover=0.0,
            rpm_cruise=2200.0,
            hub_node_id=990000 + rotor_id,
        ...
        return cls(
            rotors=rotors,
            config_type="Lift+Cruise (NASA L+C 8-rotor)",
            v_mca=18.0,           # Min control airspeed
            v_transition_end=40.0, # Transition complete at 40 m/s
            hover_ceiling_m=3000.0,
        )

    @classmethod
    def gacomp_tilt_rotor_12(cls) -> VTOLConfig:
        """Create GACOMP 12-Tilt-Rotor configuration.
        ...
        - Sized for full-model weight 15,035 N (CONM2 + structural mass):
          1g hover needs 1,253 N/rotor (disk loading 944 N/m^2,
          CT/sigma 0.135), worst hover case (1.3g vertical gust)
          needs 1,629 N/rotor (CT/sigma 0.175). BEMT capability at
          the 25 deg collective limit is ~1,848 N/rotor (+13% margin).
        - Disk diameter 1.3 m at 1.5 m span stations: 0.2 m clearance
        """
        # Tilt rotor blade. R=0.6m/c=0.05m (the L+C lift blade) saturated
        # at ~1,034 N — below even the 1g hover requirement — so the
        # blade is enlarged to R=0.65m/c=0.07m (tip Mach 0.60 at 3000rpm).
        tilt_blade = BladeDef(
            radius=0.65,
            root_cutout=0.15,
            n_elements=20,
            mean_chord=0.07,  # 70 mm = 0.07 m
            twist_root=np.radians(12.0),
            twist_tip=np.radians(3.0),
            airfoil=RotorAirfoil.naca0012(),
        )

"""Command-line interface for ASCENT-Load 3D visualization.

Usage:
    python -m ascent_load.visualization model.bdf              # View undeformed model
    python -m ascent_load.visualization model.bdf --run        # Run analysis + view results
    python -m ascent_load.visualization model.bdf --mode 1     # View mode shape
    python -m ascent_load.visualization model.bdf --aero       # View aero panels
    python -m ascent_load.visualization model.bdf --export out.vtk  # Export to VTK
    python -m ascent_load.visualization --load model.aload     # Load saved results
    python -m ascent_load.visualization --load model.aload --subcase 3
"""
...
def main():
    parser = argparse.ArgumentParser(
        prog='ascent-load-viz',
        description='ASCENT-Load 3D Model and Results Visualization',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m ascent_load.visualization cantilever.bdf            # View model
  python -m ascent_load.visualization cantilever.bdf --run      # Run + view displacement
  python -m ascent_load.visualization plate.bdf --run --modes   # Run + view modes
  python -m ascent_load.visualization goland.bdf --run --aero   # Run + view aero results
  python -m ascent_load.visualization model.bdf --screenshot fig.png
  python -m ascent_load.visualization model.bdf --export out.vtk
  python -m ascent_load.visualization --load model.aload --trim # Load saved results
  python -m ascent_load.visualization --load model.aload --loads --subcase 3
  python -m ascent_load.visualization --load model.aload --vmt  # VMT diagrams (all components)
  python -m ascent_load.visualization --load model.aload --vmt --vmt-component "Right Wing"
  python -m ascent_load.visualization --load model.aload --vmt --vmt-loads all
  python -m ascent_load.visualization --load model.aload --vmt --vmt-envelope
  python -m ascent_load.visualization --load model.aload --vmt --vmt-save vmt_plots

Certification visualization:
  python -m ascent_load.visualization --cert-vn --cert-config config.yaml
  python -m ascent_load.visualization --cert-envelope --cert-config config.yaml
  python -m ascent_load.visualization --cert-potato 5000 --cert-config config.yaml
  python -m ascent_load.visualization --cert-critical --cert-config config.yaml --cert-save out
        """,
    )

    parser.add_argument('bdf_file', nargs='?', default=None,
                        help='BDF input file')
    parser.add_argument('--load', type=str, default=None, metavar='ALOAD_FILE',
                        help='Load results from .aload file (skip BDF parsing and solving)')
    ...
    # VTOL
    parser.add_argument('--vtol', type=str, nargs='?', const='gacomp-tilt12',
                        default=None, metavar='CONFIG',
                        help='Show rotor blades (gacomp-tilt12 | gacomp-lc | JSON path)')
    ...
    if args.load:
        # Load from .aload file (no BDF parsing or solving needed)
        ...
        if not args.bdf_file:
            parser.error("bdf_file is required (or use --load ALOAD_FILE)")
    ...
    if args.vtol:
        ...
        if args.vtol == 'gacomp-tilt12':
            vtol_config = VTOLConfig.gacomp_tilt_rotor_12()
        elif args.vtol == 'gacomp-lc':
    ...
    from .viewer import AscentLoadViewer
    viewer = AscentLoadViewer(bdf_model, results, off_screen=off_screen,

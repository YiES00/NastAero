...
Converts ASCENT-Load BDFModel and SubcaseResult objects into PyVista
...
def _collect_beam_segments(bdf_model, displacements=None, scale=1.0,
    ...
    allowed = ("CBAR", "CROD") if element_types is None \
        else tuple(t for t in ("CBAR", "CROD") if t in element_types)
    for eid in sorted(bdf_model.elements.keys()):
        elem = bdf_model.elements[eid]
        if elem.type not in allowed:
            continue
...
def build_beam_tubes(bdf_model, n_sides: int = 12,
                     element_types=None) -> Optional[pv.PolyData]:
    ...
    element_types : set of str, optional
        Restrict to these types (view toggles): subset of CBAR/CROD.
...
def build_structural_mesh(bdf_model, include_beams: bool = True,
                          element_types=None) -> pv.UnstructuredGrid:
    ...
    element_types : set of str, optional
        When given, only these element types are meshed (per-type view
        toggles); membership then also governs beam/rod inclusion.
    ...
    for eid in sorted(bdf_model.elements.keys()):
        ...
        if element_types is not None:
            if etype not in element_types:
                continue
            if etype in ("CBAR", "CROD"):
                include_beams = True   # 종류 필터가 포함 여부를 결정

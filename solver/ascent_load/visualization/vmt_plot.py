...
Usage:
    from ascent_load.visualization.vmt_plot import plot_vmt_component, plot_vmt_all
...
def draw_vmt_component(fig, curves: List[VMTCurve], title: str = None) -> None:
    """Draw V/M/T subplots for one component onto an existing Figure.

    Pure Figure API (no pyplot) so it can target an embedded Qt canvas.
    """
    fig.clear()
    ...
    for curve in curves:
        ...
        axes[0].plot(curve.stations, curve.shear, label=label, **style)
        axes[1].plot(curve.stations, curve.bending_moment, label=label, **style)
        axes[2].plot(curve.stations, curve.torsion, label=label, **style)

    comp_title = title or curves[0].component_name
    axes[0].set_title(f'{comp_title} - VMT Diagram', fontsize=14,
                      fontweight='bold')
...
def draw_vmt_envelope(fig, curves: List[VMTCurve], component_name: str,
                      load_type: str = 'combined') -> None:
    """Draw a max/min VMT envelope onto an existing Figure (no pyplot)."""
    fig.clear()
...
def plot_vmt_component(
    ...
    draw_vmt_component(fig, curves, title=title)
...
def plot_vmt_envelope(
    ...
    fig = plt.figure(figsize=figsize)
    draw_vmt_envelope(fig, curves, component_name, load_type=load_type)

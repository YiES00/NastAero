...
def parse_bulk_card(fields: List[str], model: BDFModel) -> None:
    ...
    try:
        ...
        elif card_name == "AELINK":
            a = AELINK.from_fields(fields); model.aelinks.append(a)
        ...
        # DMI - direct matrix input (e.g. W2GJ camber/twist downwash)
        elif card_name == "DMI":
            from .cards.dmi import DMI
            name = fields[1].strip().upper()
            flag = fields[2].strip() if len(fields) > 2 else ""
            if flag == "0":            # 헤더 카드 — 행렬 생성
                model.dmis[name] = DMI.header_from_fields(fields)
            elif name in model.dmis:   # 컬럼 카드 — 값 채움
                model.dmis[name].fill_column(fields)
            else:
                logger.warning("DMI column card for '%s' before header "
                               "- ignored", name)
        else:

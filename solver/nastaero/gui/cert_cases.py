...
def resolve_vtol_factory(config_dict: dict):
    ...
    예: vtol_factory: nastaero.models.ilc8.make_ilc8_vtol_config
...
class CertCasesPanel(QWidget):
    ...
    def generate_cases(self) -> None:
        try:
            ...
            QMessageBox.warning(self, "NastAero", f"기체 설정 오류:\n{exc}")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "NastAero", f"케이스 생성 실패:\n{exc}")
            return
        finally:
    ...
    def export_bdf(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, "NastAero", f"BDF 생성 실패:\n{exc}")
            return

...
from qtpy.QtWidgets import (
    ...
    QLabel, QMainWindow, QMessageBox, QPlainTextEdit, QTabWidget, QVBoxLayout,
    QWidget,
)
...
_FILE_FILTER = ("NastAero files (*.bdf *.dat *.naero *.yaml *.yml);;"
                "All files (*)")
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("NastAero")
        self.resize(1500, 950)

        # 상태
        self.current_path: Optional[str] = None   # 열린 파일 (.bdf 또는 .naero)
        ...
        self._build_docks()
        self._build_menus()
        self.statusBar().showMessage("파일 > 열기로 .bdf 또는 .naero를 여세요")
    ...
    def _build_central(self) -> None:
        ...
        self.tabs.addTab(self.cert_cases_panel, "Cert Cases")
        ...
        self.tabs.addTab(self.panel_editor, "Panel")
        ...
        self.tabs.addTab(self.vmt_panel, "VMT")

        from .design_loads import DesignLoadsPanel

        self.design_loads_panel = DesignLoadsPanel()
        self.tabs.addTab(self.design_loads_panel, "Design Loads")
    ...
    def open_path(self, path: str) -> None:
        """파일 확장자에 따라 .bdf(모델) 또는 .naero(결과)를 연다."""
        if self.editor.is_dirty() and not self._confirm_discard():
            return
        p = Path(path)
        if not p.exists():
            QMessageBox.warning(self, "NastAero", f"파일이 없습니다: {path}")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "NastAero", f"열기 실패:\n{exc}")
            return
        finally:
    ...
    def _load_bdf(self, path: str) -> None:
        ...
        self._refresh_all()
        # 옆에 같은 이름의 .naero가 있으면 결과도 함께 로드
        naero = Path(path).with_suffix(".naero")
        if naero.exists():
    ...
    def _attach_results(self, naero_path: str) -> None:
        """현재 BDF 모델을 유지한 채 .naero 결과만 붙인다."""
        from ..output.result_io import load_results
    ...
    def save_bdf(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "NastAero",
                                f"저장됨. 그러나 재파싱 실패:\n{exc}")
            return
    ...
    def _confirm_discard(self) -> bool:
        answer = QMessageBox.question(
            self, "NastAero", "편집 중인 내용이 저장되지 않았습니다. 버릴까요?",
    ...
    def _update_title(self, *_args) -> None:
        ...
        self.setWindowTitle(f"NastAero — {name}{dirty}")

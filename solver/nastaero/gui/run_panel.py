...
def build_solver_command(bdf_path: str, parallel: int = 0,
    ...
    cmd = [exe, "-m", "nastaero", bdf_path, "--save",
           "--log-level", log_level]
    if parallel != 0:
...
def naero_path_for(bdf_path: str) -> str:
    """--save가 만드는 .naero 경로 (<입력>.naero)."""
    return str(Path(bdf_path).with_suffix(".naero"))
...
class RunPanel(QWidget):
    ...
    log_line = Signal(str)
    run_started = Signal()
    run_finished = Signal(str)  # 성공 시 .naero 경로, 실패 시 빈 문자열
    ...
    def set_bdf(self, bdf_path: Optional[str], sol: Optional[int]) -> None:
        """열린 BDF 경로/SOL을 반영한다. .naero 로드 시 bdf_path=None."""

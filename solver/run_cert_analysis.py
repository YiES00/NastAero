...
# Compute CLα from VLM at representative cruise Mach
# ref_area in model units (mm²) must match wing_area_m2 for Pratt consistency
wing_area_mm2 = 17.0 * 1e6  # 17.0 m² → mm²
mach_vc = 80.0 / 340.3  # VC / speed of sound ≈ 0.235
clalpha_vlm = compute_rigid_clalpha(model, mach=mach_vc, ref_area=wing_area_mm2)
print(f"    CLα (VLM, M={mach_vc:.3f}, S_ref={17.0} m²): {clalpha_vlm:.3f} /rad")
...
runner = BatchRunner(matrix, bdf_model=model, n_workers=0)

#!/usr/bin/env python3
"""Run VTOL certification loads analysis on GACOMP Lift+Cruise configuration.
...
def main():
    ...
    print("=" * 70)
    print("GACOMP Lift+Cruise VTOL Certification Loads Analysis")
    print("=" * 70)
    ...
    # 러너 입력용: 병합 전 고정익 케이스 스냅샷 (VTOL 케이스는
    # VTOLBatchRunner가 로터 하중·관성 합산 경로로 별도 처리)
    ...
    # Merge VTOL cases into conventional matrix (보고/CSV용)
    ...
    print(f"\n[8] Running batch solver: {len(conv_only.flight_cases)} "
          f"conventional + {len(vtol_cases)} VTOL cases "
          f"({n_cpus} workers)...")
    t_solve = time.time()
    ...
    runner = VTOLBatchRunner(
        ...
        n_workers=n_cpus,
        include_transient=False,  # 6-DOF 과도(OEI/잼 DAF)는 별도 실행
    )
    ...
    print(f"{'='*70}")
    print(f"  Configuration: GACOMP {vtol_config.config_type}")

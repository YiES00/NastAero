...
class TestPanelNormalFlip:
    """법선 정규화(+z/+y) 시 플립 여부 기록 — W2GJ 부호 변환의 근거."""
    ...
    def _caero(self, y1, y4):
        ...
        c.nspan = 1
        c.nchord = 1
        c.p1 = np.array([0.0, y1, 0.0])
        c.chord1 = 1.0
        c.p4 = np.array([0.0, y4, 0.0])
        c.chord4 = 1.0
        return c
    ...
    def test_right_wing_not_flipped(self):
        ...
        assert boxes[0].normal_flipped is False
    ...
    def test_mirrored_panel_flipped(self):
        ...
        assert boxes[0].normal_flipped is True  # 원 기하는 -z였음

...
class TestFullAircraftTrim:
    ...
    def test_combined_equilibrium_residuals(self):
        """트림이 강제하는 Fz/My 평형: combined 잔차 ≈ 0 이어야 한다.

        회귀 대상: ① 제약식의 패널 법선 투영 ② 트림 중량/CG와 관성력
        질량 일관성 ③ 스플라인 힘·모멘트 보존 (2026-07-08 수정 3종).
        """
        balance = self.sc.trim_balance
        weight = abs(sum(f[2]
                     for f in self.sc.nodal_inertial_forces.values()))
        refc = self.model.aeros.refc
        assert weight > 0
        assert abs(balance["Fz"]) < 1e-3 * weight, balance
        assert abs(balance["My"]) < 1e-3 * weight * refc, balance

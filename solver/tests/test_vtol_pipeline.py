...
@pytest.fixture
def gacomp_vtol_config():
    """GACOMP Lift+Cruise VTOL configuration."""
    return VTOLConfig.uam_lift_cruise()
...
class TestBEMTHover:
    ...
    def test_closed_form_bemt_validation(self, default_blade, bemt_solver):
        """Hover CT must match the classical closed-form BEMT solution.

        Independent reference path (Leishman, Principles of Helicopter
        Aerodynamics, 2nd ed., Ch. 3): per-station inflow from
            lambda(r) = sigma*a/(16 F) * (sqrt(1 + 32 F theta r/(sigma a)) - 1)
        with Prandtl tip loss F iterated via f = Nb (1-r) / (2 lambda),
        and thrust from dCT = sigma*a/2 * (theta r^2 - lambda r) dr.
        This uses only textbook equations, none of the solver's code
        paths, so agreement is a genuine cross-check (the previous
        version of this test compared the solver against a quantity
        derived from its own output and could not fail).
        """
        rho = 1.225
        rpm = 3000
        theta_col = np.radians(6.0)   # moderate collective, pre-stall
        R = default_blade.radius
        A = np.pi * R ** 2
        Nb = bemt_solver.n_blades
        sigma = default_blade.blade_solidity(Nb)
        a = 2.0 * np.pi          # solver airfoil model lift slope

        # Independent closed-form BEMT integration (no tip-loss inner
        # radii issues: integrate from root cutout to 0.99R). The local
        # pitch includes the blade's built-in twist plus collective —
        # only the blade GEOMETRY is shared with the solver; the
        # aerodynamic solution path is the textbook closed form.
        r_grid = np.linspace(default_blade.root_cutout + 1e-3, 0.99, 400)
        CT_ref = 0.0
        dr = r_grid[1] - r_grid[0]
        for r in r_grid:
            theta = default_blade.twist_at(r) + theta_col
            F = 1.0
            lam = 0.0
            for _ in range(30):
                lam = sigma * a / (16.0 * F) * (
                    np.sqrt(1.0 + 32.0 * F * theta * r / (sigma * a)) - 1.0)
                f_arg = Nb * (1.0 - r) / (2.0 * max(lam, 1e-8))
                F_new = (2.0 / np.pi) * np.arccos(np.exp(-min(f_arg, 20.0)))
                if abs(F_new - F) < 1e-8:
                    F = F_new
                    break
                F = F_new
            CT_ref += sigma * a / 2.0 * (theta * r ** 2 - lam * r) * dr

        omega = rpm * 2.0 * np.pi / 60.0
        loads = bemt_solver.solve(rpm=rpm, V_inf=0.0, rho=rho,
                                  collective_rad=theta_col)
        CT_solver = loads.thrust / (rho * A * (omega * R) ** 2)

        assert CT_ref > 1e-4  # sanity: reference produces real thrust
        assert CT_solver == pytest.approx(CT_ref, rel=0.10)
...
class TestVTOLConfig:
    """Tests for GACOMP Lift+Cruise VTOL configuration."""

    def test_gacomp_rotor_count(self, gacomp_vtol_config):
        assert len(gacomp_vtol_config.rotors) == 7

    def test_gacomp_lift_rotors(self, gacomp_vtol_config):
        lift = gacomp_vtol_config.lift_rotors
        assert len(lift) == 6
        for r in lift:
            assert r.rotor_type == RotorType.LIFT

    def test_gacomp_cruise_rotors(self, gacomp_vtol_config):
        cruise = gacomp_vtol_config.cruise_rotors
        assert len(cruise) == 1
        assert cruise[0].rotor_type == RotorType.CRUISE

    def test_gacomp_config_type(self, gacomp_vtol_config):
        assert gacomp_vtol_config.config_type == "Lift+Cruise"

    def test_rotor_alternating_rotation(self, gacomp_vtol_config):
    ...
    def test_rotor_hub_nodes_unique(self, gacomp_vtol_config):
        hub_nodes = [r.hub_node_id for r in gacomp_vtol_config.rotors]
        assert len(set(hub_nodes)) == len(hub_nodes)

    def test_total_rotor_mass(self, gacomp_vtol_config):
        total = gacomp_vtol_config.total_rotor_mass_kg
        assert total > 0.0
        # 6 lift rotors × 15kg + 1 cruise × 12kg = 102 kg
        assert total == pytest.approx(102.0, rel=0.01)

    def test_rotor_positions_symmetric(self, gacomp_vtol_config):
...
class TestRotorLoadsApplicator:
    ...
    @pytest.fixture
    def test_rotor(self, gacomp_vtol_config):
        return gacomp_vtol_config.lift_rotors[0]
    ...
    def test_all_rotor_forces(self, gacomp_vtol_config, test_loads):
        loads_map = {}
        for r in gacomp_vtol_config.rotors:
            loads_map[r.rotor_id] = test_loads

        combined = all_rotor_forces(gacomp_vtol_config.rotors, loads_map)
...
class TestVTOLLoadCaseMatrix:
    ...
    def test_matrix_generates_cases(self):
        ...
        vtol_config = VTOLConfig.uam_lift_cruise()
...
class TestTransitionGustConditions:
    """천이 회랑 준정적 돌풍 케이스 — 호버(±0.3)와 고정익 Pratt 사이."""

    ARGS = dict(v_mca=15.0, v_transition_end=35.0, altitudes_m=[0.0],
                wing_area_m2=16.2, CL_transition=1.0, weight_N=18633.0,
    ...
    def test_count_and_pairing(self):
        conds = self._conds()
        assert len(conds) == 16          # 8 속도점 x 상승/하강
        ups = [c for c in conds if "gust+" in c.label]
        downs = [c for c in conds if "gust-" in c.label]
        assert len(ups) == len(downs) == 8

    def test_hover_end_blends_to_rotor_gust(self):
        """최저 속도(로터 분담~1)에서 dn이 호버 관례 0.3에 접근."""
        conds = self._conds()
        lo = min(conds, key=lambda c: c.V_eas)
        assert lo.nz > 1.25              # 0.3(로터) + 소량 Pratt

    def test_wing_end_pratt_dominates(self):
        """고속 끝(날개 전담 근접)에서 Pratt 성분이 지배 — dn 증가."""
        conds = [c for c in self._conds() if "gust+" in c.label]
        lo = min(conds, key=lambda c: c.V_eas)
        hi = max(conds, key=lambda c: c.V_eas)
        assert hi.nz > lo.nz

    def test_downgust_thrust_never_negative(self):
        for c in self._conds():
            assert c.thrust_fraction >= 0.0
    ...
    def test_matrix_includes_gust_cases(self):
        ...
        cfg = AircraftConfig(
            speeds=SpeedSchedule(VS1=36.0, VA=71.0, VB=0.0, VC=80.0,
                                 VD=100.0, VF=50.0),
            weight_cg_conditions=[WeightCGCondition(
                label="MTOW", weight_N=18633.0, cg_x=4450.0)],
            altitudes_m=[0.0], wing_area_m2=16.2, CLalpha=5.348,
            mean_chord_m=1.35)
        matrix = VTOLLoadCaseMatrix(make_ilc8_vtol_config(), cfg)
        cases = matrix.generate_all()
        gust = [c for c in cases
                if "Transition gust" in c.trim_condition.label]
        assert len(gust) == 16
        assert len(cases) == 55
